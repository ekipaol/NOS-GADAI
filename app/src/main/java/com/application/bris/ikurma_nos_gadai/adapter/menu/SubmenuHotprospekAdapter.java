...
//        flagScoring = 1;
        ...
        if (data.getId_st_aplikasi() == 1 || data.getId_st_aplikasi() == -14){
            ...
            if (position !=7){
                ...
                holder.iv_iconmenu.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                        AppUtil.notifinfo(context,holder.rl_menu.getRootView().findViewById(android.R.id.content),"Tidak bisa mengakses menu ini jika aplikasi sudah dikirim ke pemutus");
                    }
                });
                setGreyorRegularIcon(holder.iv_iconmenu, 0);
            }
    ...
    public void validateClick(RelativeLayout rl, ImageView ivComplete, ImageView ivMenu, int pos){
        ...
        //ini supaya scoring tetap enabled walaupun agunan tidak ada flagging ceklis
        else if (flagFinansial == 1 && pos == 5){
            rl.setEnabled(true);
            setGreyorRegularIcon(ivMenu, 1);
        }
