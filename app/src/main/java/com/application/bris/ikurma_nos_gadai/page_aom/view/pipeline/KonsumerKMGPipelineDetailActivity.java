...
public class KonsumerKMGPipelineDetailActivity extends AppCompatActivity implements View.OnClickListener, ConfirmListener {
    ...
    private KonsumerKMGDetailPipeline data;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.konsumer_kmg_pipeline_detail);
    ...
    private void loadDataPipeline() {
        ...
        Call<ParseResponse> call = apiClientAdapter.getApiInterface().inquiryPipelineKonsumer(req);
...
//                                Log.d("detail rekanan", data.getREKANAN_DM());
                                ...
                                if (data.getKode_gimmick() == 1 || data.getKode_gimmick() == 26 || data.getKode_gimmick() == 27 || data.getKode_gimmick() == 28) {
                                ...
                                TindaklanjutAdapater adp = new TindaklanjutAdapater(KonsumerKMGPipelineDetailActivity.this, tindaklanjuts);
                                rv_history_tindaklanjut.setLayoutManager(new LinearLayoutManager(KonsumerKMGPipelineDetailActivity.this));
                                rv_history_tindaklanjut.setItemAnimator(new DefaultItemAnimator());
                                rv_history_tindaklanjut.setAdapter(adp);
                            }
                        }
                        else {
                            AppUtil.notiferror(KonsumerKMGPipelineDetailActivity.this, findViewById(android.R.id.content), response.body().getMessage());
                        }
                    }
                    else {
                        Error error = ParseResponseError.confirmEror(response.errorBody());
                        AppUtil.notiferror(KonsumerKMGPipelineDetailActivity.this, findViewById(android.R.id.content), error.getMessage());
                    }
                }
                catch (Exception e){
                    e.printStackTrace();
            ...
            public void onFailure(Call<ParseResponse> call, Throwable t) {
                loading.setVisibility(View.GONE);
                AppUtil.notiferror(KonsumerKMGPipelineDetailActivity.this, findViewById(android.R.id.content), getString(R.string.txt_connection_failure));
            }
        });
    }
    ...
    private void sendData(final String type){
        ...
        if (type.equalsIgnoreCase(getString(R.string.title_gotohotprospek))){
            call = apiClientAdapter.getApiInterface().pipelineToHotprospekKonsumer(req);
        }
        else if(type.equalsIgnoreCase("Hapus Pipeline")){
            call = apiClientAdapter.getApiInterface().rejectPipelineKonsumer(req);
        }
        call.enqueue(new Callback<ParseResponse>() {
            @Override
            public void onResponse(Call<ParseResponse> call, Response<ParseResponse> response) {
                ...
                try {
                    if (response.isSuccessful()){
                        if(response.body().getStatus().equalsIgnoreCase("00")){
                            ...
                            CustomDialog.DialogSuccess(KonsumerKMGPipelineDetailActivity.this, "Success!", msg, KonsumerKMGPipelineDetailActivity.this);
                        }
                        else {
                            AppUtil.notiferror(KonsumerKMGPipelineDetailActivity.this, findViewById(android.R.id.content), response.body().getMessage());
                        }
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btnfab_edit:
                ...
                Intent it = new Intent(this, KonsumerKMGPipelineEditActivity.class);
