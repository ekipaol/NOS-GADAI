...
public class BSRejectHotprospek extends BottomSheetDialogFragment implements View.OnClickListener, ConfirmListener{
    ...
    public static BSRejectHotprospek display(FragmentManager fragmentManager, Context mcontex){
        context = mcontex;
        BSRejectHotprospek BSCekNasabah = new BSRejectHotprospek();
    ...
    public void sendReject(){
        ...
        rejectHotprospek req = new rejectHotprospek(HotprospekDetailActivity.idAplikasi, String.valueOf(appPreferences.getUid()), keterangan);
        Call<ParseResponse> call = apiClientAdapter.getApiInterface().rejectHotprospek(req);
                ...
                try {
                    if (response.isSuccessful()){
                        if (response.body().getStatus().equalsIgnoreCase("00")){
                            CustomDialog.DialogSuccess(getContext(), "Success!", "ID Aplikasi "+String.valueOf(HotprospekDetailActivity.idAplikasi)+" Sukses ditolak", BSRejectHotprospek.this);
                        }
                        else{
