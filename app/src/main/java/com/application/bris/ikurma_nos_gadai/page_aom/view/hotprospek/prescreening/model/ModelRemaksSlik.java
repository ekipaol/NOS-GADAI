package com.application.bris.ikurma_nos_gadai.page_aom.view.hotprospek.prescreening.model;

import com.google.gson.annotations.SerializedName;

/**
 * Created by idong on 13/05/2019.
 */

public class ModelRemaksSlik {
    @SerializedName("OS")
    public Long oS;
    @SerializedName("FLAG_TAKEOVER")
    public Integer fLAGTAKEOVER;
    @SerializedName("TANGGAL_JATUH_TEMPO")
    public String tANGGALJATUHTEMPO;
    @SerializedName("KONDISI")
    public String kONDISI;
    @SerializedName("KOL")
    public Integer kOL;
    @SerializedName("FID_PHOTO")
    public Integer fIDPHOTO;
    @SerializedName("KONDISI_KET")
    public String kONDISIKET;
    @SerializedName("PLAFOND")
    public Long pLAFOND;
    @SerializedName("TANGGAL_KONDISI")
    public String tANGGALKONDISI;
    @SerializedName("ANGSURAN")
    public Double aNGSURAN;
    @SerializedName("REMARK_KET")
    public String rEMARKKET;
    @SerializedName("ID")
    public Integer iD;
    @SerializedName("REMARK")
    public Integer rEMARK;
    @SerializedName("NAMA_BANK")
    public String nAMABANK;
    @SerializedName("TANGGAL_MULAI")
    public String tANGGALMULAI;
    @SerializedName("KODE_BANK")
    public String kODEBANK;

    public Long getoS() {
        return oS;
    }

    public void setoS(Long oS) {
        this.oS = oS;
    }

    public Integer getfLAGTAKEOVER() {
        return fLAGTAKEOVER;
    }

    public void setfLAGTAKEOVER(Integer fLAGTAKEOVER) {
        this.fLAGTAKEOVER = fLAGTAKEOVER;
    }

    public String gettANGGALJATUHTEMPO() {
        return tANGGALJATUHTEMPO;
    }

    public void settANGGALJATUHTEMPO(String tANGGALJATUHTEMPO) {
        this.tANGGALJATUHTEMPO = tANGGALJATUHTEMPO;
    }

    public String getkONDISI() {
        return kONDISI;
    }

    public void setkONDISI(String kONDISI) {
        this.kONDISI = kONDISI;
    }

    public Integer getkOL() {
        return kOL;
    }

    public void setkOL(Integer kOL) {
        this.kOL = kOL;
    }

    public Integer getfIDPHOTO() {
        return fIDPHOTO;
    }

    public void setfIDPHOTO(Integer fIDPHOTO) {
        this.fIDPHOTO = fIDPHOTO;
    }

    public String getkONDISIKET() {
        return kONDISIKET;
    }

    public void setkONDISIKET(String kONDISIKET) {
        this.kONDISIKET = kONDISIKET;
    }

    public Long getpLAFOND() {
        return pLAFOND;
    }

    public void setpLAFOND(Long pLAFOND) {
        this.pLAFOND = pLAFOND;
    }

    public String gettANGGALKONDISI() {
        return tANGGALKONDISI;
    }

    public void settANGGALKONDISI(String tANGGALKONDISI) {
        this.tANGGALKONDISI = tANGGALKONDISI;
    }

    public Double getaNGSURAN() {
        return aNGSURAN;
    }

    public void setaNGSURAN(Double aNGSURAN) {
        this.aNGSURAN = aNGSURAN;
    }

    public String getrEMARKKET() {
        return rEMARKKET;
    }

    public void setrEMARKKET(String rEMARKKET) {
        this.rEMARKKET = rEMARKKET;
    }

    public Integer getiD() {
        return iD;
    }

    public void setiD(Integer iD) {
        this.iD = iD;
    }

    public Integer getrEMARK() {
        return rEMARK;
    }

    public void setrEMARK(Integer rEMARK) {
        this.rEMARK = rEMARK;
    }

    public String getnAMABANK() {
        return nAMABANK;
    }

    public void setnAMABANK(String nAMABANK) {
        this.nAMABANK = nAMABANK;
    }

    public String gettANGGALMULAI() {
        return tANGGALMULAI;
    }

    public void settANGGALMULAI(String tANGGALMULAI) {
        this.tANGGALMULAI = tANGGALMULAI;
    }

    public String getkODEBANK() {
        return kODEBANK;
    }

    public void setkODEBANK(String kODEBANK) {
        this.kODEBANK = kODEBANK;
    }
}
