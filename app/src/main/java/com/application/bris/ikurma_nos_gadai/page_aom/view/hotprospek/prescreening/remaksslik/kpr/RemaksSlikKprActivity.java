...
public class RemaksSlikKprActivity extends AppCompatActivity {
    ...
    private void loadData() {
        ...
        Call<ParseResponse> call = apiClientAdapter.getApiInterface().inquiryRemaksSlikKpr(req);
        call.enqueue(new Callback<ParseResponse>() {
                ...
                try {
                    if (response.isSuccessful()) {
                        if (response.body().getStatus().equalsIgnoreCase("00")) {
                            Gson gson = new Gson();
                            Type type = new TypeToken<List<ModelRemaksSlik>>() {
                            }.getType();
                            if (statusKawin == 2) {
                            ...
                            AppUtil.notiferror(RemaksSlikKprActivity.this, findViewById(android.R.id.content), response.body().getMessage());
                            finish();
                        }
                    } else {
                        Error error = ParseResponseError.confirmEror(response.errorBody());
                        AppUtil.notiferror(RemaksSlikKprActivity.this, findViewById(android.R.id.content), error.getMessage());
                        finish();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            public void onFailure(Call<ParseResponse> call, Throwable t) {
                loading.setVisibility(View.GONE);
                AppUtil.notiferror(RemaksSlikKprActivity.this, findViewById(android.R.id.content), getString(R.string.txt_connection_failure));
                finish();
            }
        });
    ...
    class ViewPagerAdapter extends FragmentStatePagerAdapter {
        ...
        public Fragment getItem(int position) {
            ...
            if (statusKawin == 2) {
                switch (position) {
                    case 0:
                        fr = new FragmentRemaksSlikNasabahKpr();
                    ...
                    case 1:
                        fr = new FragmentRemaksSlikPasanganKpr();
